 ...
 * "ceiling" é sempre o nível INTERNO (numeração NASA/ISO, 1 a 9) — o
 * rótulo exibido ao usuário é recalculado por régua (ver toDisplay). */
const TRL_GROUPS = [
  { id: 1, ceiling: 3, name: "Pesquisa e Desenvolvimento (P&D)",
  ...
  { id: 2, ceiling: 6, name: "Construção da Tecnologia",
  ...
  { id: 3, ceiling: 9, name: "Validação e Produção",
...
/* ===================================================================
 * Réguas de maturidade (frameworks)
 *
 * O checklist (TRL_LEVELS) é único — os mesmos critérios e nomes de
 * nível valem para qualquer régua. O que muda de uma régua para outra
 * é só a NUMERAÇÃO exibida, via um deslocamento (offset) fixo entre a
 * numeração interna (NASA/ISO, 1 a 9) e a numeração da régua.
 *
 * API 17N (American Petroleum Institute, Recommended Practice 17N,
 * 2009 — indústria subsea de óleo & gás) usa uma escala de 8 níveis
 * (TRL 0 a 7) que corresponde à escala NASA deslocada em 1 nível:
 * TRL(API) = TRL(NASA) − 1. O TRL 9 (NASA) não tem correspondente na
 * régua API, que termina em TRL 7 — correspondência de níveis
 * conforme diagrama comparativo de referência da indústria (TRL
 * genérico ↔ API 17N Technology Readiness Level).
 * =================================================================== */
const FRAMEWORKS = {
  nasa: {
    id: "nasa",
  ...
  api: {
    ...
    scaleNote: "8 níveis (TRL 0–7) — mesmos critérios da régua NASA/ISO 16290 acima, renumerados conforme a correspondência API 17N (TRL API = TRL NASA − 1). O TRL 9 (NASA) não tem correspondente na régua API.",
    citation: "API Recommended Practice 17N (2009); correspondência de níveis conforme diagrama comparativo de referência da indústria (TRL genérico ↔ API 17N).",
    offset: -1,
    maxInternalLevel: 8
  }
};
